...
| :--- | :--- | :--- |
| (1), (2) | **BR-050** | **Displaying Rules:**<br/>The system initializes the Order Form by first retrieving the currently authenticated user from the session. It then fetches all items from the user's shopping cart using their customer ID. If the cart is found to be empty, the system displays an informational message (MSG-020) and redirects the user back to the home page, as there are no items to process. If the cart contains items, the system populates the order form with the customer's stored address information and displays all cart items with their details (name, quantity, price) for review before submission.<br/><br/>customer = session.getCurrentUser()<br/>cartItems = database.getCart(customer.id)<br/><br/>IF cartItems IS EMPTY:<br/>displayMessage(MSG-020)<br/>redirect(HomePage)<br/><br/>ELSE:<br/>form.display(customer.address)<br/>form.display(cartItems) |
| (3) | **BR-051** | **Validation Rules:**<br/>Triggered when the user clicks the [Submit Order] button. The system first captures all data entered in the form, then performs a series of validation checks to ensure the order can be processed. It verifies that the shipping address field has been filled in, as this is mandatory for delivery. Next, it checks that the user has selected a payment method from the available options. Finally, the system queries the database to verify that all items in the cart are currently in stock. If any of these validations fail, the system displays the appropriate error message and stops the submission process, allowing the user to correct the issue before trying again.<br/><br/>input = form.getData()<br/><br/>IF input.shippingAddress IS EMPTY:<br/>displayError(MSG-005)<br/>RETURN False<br/><br/>IF input.paymentMethod IS NOT SELECTED:<br/>displayError(MSG-006)<br/>RETURN False<br/><br/>inventoryStatus = database.checkStock(cartItems)<br/>IF inventoryStatus IS "Out of Stock":<br/>displayError(MSG-099)<br/>RETURN False |
| (4), (5) | **BR-052** | **Processing Rules:**<br/>After all validations pass, the system processes the valid order by creating a new order record in the system. It begins by instantiating a new order object and associating it with the customer and their shipping address. The system then iterates through each item in the shopping cart, creating individual line items that capture the product ID, price, and quantity ordered. As each line item is added to the order, the system simultaneously reduces the available stock quantity in the inventory database to reflect the purchase. Once all items are processed, the system calculates the order total (including any applicable taxes or fees) and assigns it to the order. The system then attempts to save the complete order to the database within a transaction. Upon successful save, a confirmation email is sent to the customer's email address and a success message (MSG-100) is displayed. If any error occurs during the save or email process (such as database connection issues or email service failures), the system logs the error details for troubleshooting and displays a generic error message (MSG-500) to the user.<br/><br/>order = createNewOrder()<br/>order.setCustomer(customer)<br/>order.setAddress(input.shippingAddress)<br/><br/>FOR EACH item IN cartItems:<br/>lineItem = createLineItem(item.id, item.price, item.qty)<br/>order.addItem(lineItem)<br/>database.reduceStock(item.id, item.qty)<br/><br/>total = calculateTotal(order)<br/>order.setTotal(total)<br/><br/>TRY:<br/>database.save(order)<br/>emailService.sendConfirmation(customer.email)<br/>displaySuccess(MSG-100)<br/>CATCH Exception:<br/>logError(Exception)<br/>displayError(MSG-500) |
